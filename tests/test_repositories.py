...
from seahorse.infrastructure.repositories.user_profile_json import JSONUserProfileRepository
...
def test_storage_paths_build_from_config(tmp_path: Path) -> None:
    ...
    assert paths.user_profile_path == tmp_path / "memory-data" / USER_PROFILE_FILE_NAME


def test_user_profile_repository_returns_none_when_missing(tmp_path: Path) -> None:
    repository = JSONUserProfileRepository(tmp_path / "data" / USER_PROFILE_FILE_NAME)

    assert repository.load() is None


def test_user_profile_repository_saves_and_loads_json(tmp_path: Path) -> None:
