...
class JSONUserProfileRepository:
    ...
    def _load_json(self) -> UserProfile | None:
        try:
            ...
            raise RuntimeError(f"Failed to read user profile storage: {self._path}") from exc

        try:
            return UserProfile.model_validate_json(raw_content)
        except ValidationError as exc:
            raise RuntimeError("User profile storage contains invalid JSON") from exc
    ...
    def save(self, model: UserProfile) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to write user profile storage: {self._path}") from exc
